...
export type SimulatorTestPageProps = {
  topic: TopicItem;
}

const SimulatorTestPage = (props: PropsWithoutRef<SimulatorTestPageProps>) => {
  const appInfo = useSelector((state) => state.appInfos.appInfo);
  const { loading: authLoading } = useSelector((state) => state.authState);
  const { currentTopic } = useSelector((state) => state.topicState);
...
export const getServerSideProps = wrapper.getServerSideProps(async ({ query, store }) => {
  ...
  if (appName !== "toeic") return { notFound: true };
